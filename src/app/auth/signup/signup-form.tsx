...
export default function SignUpForm({
  ...
  const callbackUrl = role === 'EMPLOYER' ? '/employers/plans' : '/jobs'
  ...
  async function handleGoogleClick() {
    ...
    setLoading(true)
    const res = await fetch('/api/auth/demo-google', { method: 'POST' })
    ...
    setLoading(false)
    if (result?.error) {
      setError('Demo sign-in failed. Please try email/password.')
    } else {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
          ...
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 border border-gray-200 rounded-xl py-2.5 px-4 font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-60 transition-colors mb-6"
          >
        ...
        <p className="text-center text-sm text-gray-500 mt-6">
          ...
          <Link href="/auth/signin" className="text-teal-600 font-medium hover:underline">Sign in</Link>
        </p>
      </div>
    </div>
  )
